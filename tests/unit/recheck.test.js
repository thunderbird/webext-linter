...
// ---- resolvePermissionRecheck: per-occurrence aggregation ----
// A handed permission item carrying its located token sites (occurrences). The summary
// verdicts each SITE by its id; resolvePermissionRecheck aggregates per permission.
const permItem = (permission, occ, line = 6) => ({
...
const site = (id, file, line, token) => ({ id, file, line, token });
const permCtx = (handedItems, recheck) =>
  withManifest({
    recheck: new Map([["unused-permission-recheck", handedItems]]),
    addon: { recheck },
  });

// Any site judged pass -> the permission is exercised -> dropped, even amid fails.
test("resolvePermissionRecheck drops a permission when any of its sites passes", () => {
  const ctx = permCtx(
    [
      permItem("compose", [
        site("compose#1", "a.js", 1, "executeScript"),
        site("compose#2", "b.js", 2, "insertCSS"),
      ]),
    ],
    [
      {
  ...
  assert.deepEqual(out.escalations, []); // a passing site justifies the whole permission
});

// Every site definitively not exercising it (all fail) -> a warning finding on the
// permission, located at its manifest line (not a site line).
test("resolvePermissionRecheck flags a permission when every site fails", () => {
  const ctx = permCtx(
    [
      permItem("compose", [
        site("compose#1", "a.js", 1, "executeScript"),
  ...
  assert.equal(out.findings[0].loc.line, 6); // the permission's manifest line
  assert.deepEqual(out.escalations, []);
});

// Anything else - a mix of fail/unsure, all unsure, or no verdict (summary skipped) -
// falls to manual review, so an uncertain permission is never silently dropped or flagged.
test("resolvePermissionRecheck routes a mixed/uncertain permission to manual", () => {
  const ctx = permCtx(
    [
      permItem("compose", [
        site("compose#1", "a.js", 1, "executeScript"),
      ...
      permItem("cookies", [site("cookies#1", "c.js", 3, "cookieStoreId")], 7),
    ],
    [
      {
        check: "unused-permission-recheck",
  ...
  assert.deepEqual(
    out.escalations.map((e) => e.item).sort(),
    ["compose", "cookies"] // fail+unsure mix, and the no-verdict permission, both manual
  );
});

// A token-less permission (no occurrences) is judged holistically: its verdict is keyed
// by the permission itself, and drives drop/finding/manual directly.
test("resolvePermissionRecheck judges a token-less permission holistically", () => {
  const ctx = permCtx(
    [
      permItem("unlimitedStorage", []), // no sites -> holistic
      permItem("management", []),
    ],
    [
      {
  ...
  assert.deepEqual(
    out.findings.map((f) => f.item),
    ["management"] // holistic fail -> finding
  );
  assert.deepEqual(out.escalations, []); // unlimitedStorage holistic pass -> dropped
});

// The guard: a verdict for a site id never handed over (a model invention) is inert.
test("resolvePermissionRecheck ignores verdicts for sites it was not handed", () => {
  const ctx = permCtx(
    [permItem("compose", [site("compose#1", "a.js", 1, "executeScript")])],
    [
      {
        check: "unused-permission-recheck",
        item: "compose#1",
  ...
  assert.deepEqual(out.findings, []);
  assert.deepEqual(out.escalations, []); // only compose#1 (pass) is consulted -> dropped
});
...
// The script-injection entries make the tab-kind-ambiguous injection permissions
// recheckable - a permission-prompts entry is the only thing that lets the LLM re-judge them
// (otherwise an injection-only add-on leaves them ungrounded -> straight to manual). NOT
// scripting: it is namespace-guarded (any scripting.* call grounds it deterministically), so
// it has no ambiguous case and no entry.
test("the tab-kind injection permissions (activeTab, compose, messagesModify) are recheckable", () => {
  const recheckable = loadRegistry().recheckablePermissions();
  for (const p of ["activeTab", "compose", "messagesModify"]) {
    assert.ok(recheckable.has(p), `expected "${p}" to be recheckable`);
  }
  // scripting is grounded deterministically (namespace permission), so it is NOT recheckable.
  assert.ok(!recheckable.has("scripting"), "scripting must not be recheckable");
});
...
test("permissionPrompts surfaces the optional usage tokens", () => {
  ...
  // Dotted (api-resolved) injection tokens + the bare `attachments` property token.
  // compose_scripts is NOT a token: the manifest key grounds compose deterministically.
...
// The permission-prompts token vocabulary is attached at load to the PRODUCER, which
// both locates the token sites and renders the deterministic verdicts from it. The
// consumer reads the prompts straight off the registry when it assembles its rubric,
// so it needs no copy. Only the token entries are attached (prompt prose stripped), so
// the producer has no window into the consumer's wording. A static-rubric pair carries none.
test("loadChecks attaches the permission-prompts vocabulary to the producer only", async () => {
  ...
  // Deliberately narrow: the token entries only - no prompt prose.
  ...
  // The consumer needs no copy - it reads the registry directly.
...
test("assembly drops a handed permission no version-matching prompt grounds", () => {
  ...
  // so it has no verdict and resolvePermissionRecheck routes it to manual review.
  const resolved = resolvePermissionRecheck(ctx, {
    id: "unused-permission-recheck",
  });
  assert.ok(
...
test("the assembled rubric always carries the framing verdict scheme", () => {
  ...
  // The candidate framing (a site is a given candidate, not an open "is it used"), the
  // discriminator, and the verdict mechanics.
  assert.match(rubric, /A code SITE/);
  assert.match(rubric, /genuine CANDIDATE - do not dismiss it/);
  assert.match(rubric, /pass = this site meets the criteria/);
  assert.match(rubric, /unsure is the default/);
  assert.match(rubric, /Ignore comments, TODOs/); // the judge-current-code-only rule
  // The code-inserted "Sites to judge" list and the closing discriminator instruction.
  assert.match(rubric, /Sites to judge/);
  assert.match(rubric, /answer only the discriminator/);
  // No leftover citation language.
  assert.doesNotMatch(rubric, /usages/);
  assert.doesNotMatch(rubric, /Accepted tokens/);
});
