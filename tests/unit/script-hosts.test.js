...
// Gecko uses to resolve a relative page-relative loader path (tabs.create /
// windows.create {url}, tabs.executeScript/insertCSS/removeCSS {file}, menus
...
// ".." is clamped at the package root (Gecko's `new URL(path, baseURI)` cannot
// climb above the origin), not treated as a wrong path. A module in a subdir
// imported by a root-hosted background script gets base "" (root), so "../x"
// clamps to "x" and surplus ".." saturate - they resolve to the bundled file
// rather than escaping. This is the qnote/filtaquilla shape (an icon "../skin/x"
// that loads).
test("a leading .. clamps at root for a root-hosted (background.scripts) module", () => {
  const manifest = {
    manifest_version: 2,
  ...
  const files = {
    "manifest.json": JSON.stringify(manifest),
    "background.mjs": `import "./modules/menu.mjs";`,
    "modules/menu.mjs": `browser.menus.create({ icons: "../skin/x.svg" });`,
    "skin/x.svg": ``,
  };
  const ctx = ctxFrom(files, manifest);
  const hd = scriptHostDirs(ctx);
  ...
  assert.equal(
    resolvePageRelative(f, hd, "modules/menu.mjs", "../skin/x.svg"),
    "skin/x.svg"
  );
  // Surplus ".." saturate at root.
  assert.equal(
    resolvePageRelative(f, hd, "modules/menu.mjs", "../../../skin/x.svg"),
    "skin/x.svg"
  );
...
// From a subdir host page, a single ".." is a real directory step (walk out,
// then back in), while surplus ".." clamp at root - the nested-page shape.
test(".. walks out of a subdir host page then clamps at root", () => {
  const manifest = {
    manifest_version: 2,
    background: { page: "nested/background.html" },
  };
  const files = {
    "manifest.json": JSON.stringify(manifest),
    "nested/background.html": `<script type="module" src="menu.js"></script>`,
    "nested/menu.js": `browser.menus.create({ icons: "x.svg" });`,
    "nested/icons/a.svg": ``,
    "top.svg": ``,
  };
  const ctx = ctxFrom(files, manifest);
  ...
  assert.equal(
    resolvePageRelative(f, hd, "nested/menu.js", "icons/a.svg"),
    "nested/icons/a.svg"
  );
  // base nested/: one ".." walks out to root, reaching a root-level file.
  assert.equal(
    resolvePageRelative(f, hd, "nested/menu.js", "../top.svg"),
    "top.svg"
  );
  // base nested/: walk out then back in.
  assert.equal(
    resolvePageRelative(f, hd, "nested/menu.js", "../nested/icons/a.svg"),
    "nested/icons/a.svg"
  );
  // surplus ".." clamp at root.
  assert.equal(
    resolvePageRelative(f, hd, "nested/menu.js", "../../top.svg"),
    "top.svg"
  );
