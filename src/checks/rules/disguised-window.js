// Data smuggled out through a window.open() to a remote URL built with appended
...
export default {
  run(ctx) {
    ...
    for (const sink of getOutboundSinks(ctx)) {
      if (sink.type !== "window-open" || !isCovertExfil(sink)) {
        continue;
      }
      const loc = { line: sink.line, column: sink.column };
      out.push(finding({ file: sink.file, loc }));
      ctx.note?.(sink.file, loc, "disguised data send (window-open)", "fail");
    }
    return out;
  },
};
