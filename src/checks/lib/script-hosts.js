...
// tabs.executeScript / insertCSS / removeCSS `file` path. In Firefox/Thunderbird
// such a path is resolved against the "current page URL" (the document the
// calling code runs in), NOT the extension root (Chrome's rule) and NOT the
// calling script's own URL (an `import`'s rule). So a script's resolution base
// is its HOST PAGE's directory.
//
...
/**
 * Resolve a page-relative loader path (a tabs.executeScript/insertCSS/removeCSS
 * `file`) against the host page(s) of the calling script. With a known host
 * page, resolve against each host directory and return the first packaged hit,
 * else null - a genuine "not bundled" reference is still caught. With NO known
 * host page (e.g. the call sits in a script no declared page loads), fall back
 * to root-relative, so behavior never regresses below the old root-only rule.
