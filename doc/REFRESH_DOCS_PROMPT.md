...
documentation site under `doc/` so it stays in sync with the checks the tool
actually implements.
...
Update the static documentation site in `doc/` so that it documents **all
...
- `doc/index.html` — the shell. A left sidebar lists every check (grouped by
...
- `doc/assets/style.css` — shared styling for the shell and the check pages.
- `doc/assets/mermaid.min.js` — Mermaid, **vendored locally** (offline). Do not
  switch to a CDN.
- `doc/checks/<id>.html` — one standalone page per check.
...
2. **Diff against the site.** Compare that list to the `CHECKS` array in
   `doc/index.html` and the files in `doc/checks/`. Identify: new checks (need a
...
3. **Author / update one page per check** at `doc/checks/<id>.html`, following
   the existing page template exactly (see any current page, e.g.
   `doc/checks/core-symbol-in-webext.html`). Each page has:
...
5. **Update the sidebar.** Rebuild the `CHECKS` array in `doc/index.html` so it
...
7. **Verify.** Confirm there is exactly one `doc/checks/<id>.html` per check in
   ...
   screenshot tool is available, open `doc/index.html` and spot-check a few
